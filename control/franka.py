...
class RobotController:
    ...
    def _ensure_sim_context(self) -> None:
        """Check for SimulationContext but do NOT create one on-the-fly.

        Creating a SimulationContext while Kit's timeline is already playing can
        interfere with native physics stepping.  The robot controller works fine
        without it (falls back to dt=1/60).
        """
        try:
            from isaaclab.sim import SimulationContext
            ctx = SimulationContext.instance()
            if ctx is not None:
                self._log("[INFO] SimulationContext found; using it for physics dt.")
            else:
                self._log("[INFO] No SimulationContext — using Kit-native physics (dt fallback).")
        except Exception:
            self._log("[INFO] SimulationContext unavailable — using Kit-native physics.")
