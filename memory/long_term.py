...
class _Mem0Adapter:
    ...
    def _is_mem0_bad_request(self, exc: Exception) -> bool:
        msg = str(exc).lower()
        return "400 bad request" in msg or ("status code: 400" in msg and "mem0" in msg)
            ...
            if hasattr(self._mem, "add"):
                ...
                for fn in add_attempts:
                    try:
                        ...
                        break
                    except TypeError as exc:  # pragma: no cover - signature compatibility path
                        # Try another signature variant only for local call-shape errors.
                        last_exc = exc
                        continue
                    except Exception as exc:
                        # Remote/API errors should not trigger repeated retries.
                        last_exc = exc
                        break
    ...
    def retrieve(self, query: str, top_k: int = 5) -> List[MemoryItem]:
        ...
        try:
            ...
            if hasattr(self._mem, "search"):
                search_attempts = (
                    # Prefer user_id-only forms first; some Mem0 deployments reject extra identity fields.
            ...
            if self._is_mem0_identity_error(exc):
                self._disable_mem0(str(exc))
                return self._fallback.retrieve(query, top_k=top_k)
            if self._is_mem0_bad_request(exc):
                # Treat repeated 400s as incompatible backend configuration to avoid log spam.
                self._disable_mem0(str(exc))
                return self._fallback.retrieve(query, top_k=top_k)
